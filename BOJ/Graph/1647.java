...
/* 문제 해설
 MST로 연결된 마을을 2개로 분할하여 도로 유지비를 최소로 유지하려면,
 엣지 1개를 끊고 집 1개로 구성된 마을1과 나머지 집으로 구성된 마을2로 분할하되, 끊어낸 하나의 엣지가 가장 커야함
 즉, MST를 이루는 엣지 중 가장 큰 것을 잘라내면 됨
 */
	...
	public static void kruskal() {
		...
		while(!pq.isEmpty()){
			...
			if(find(now.v1)!=find(now.v2)){
				...
				maxWeight = Math.max(now.weight, maxWeight);
				union(now.v1, now.v2);
				count++;
			}
